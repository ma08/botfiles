---
name: oracle-awaiter
description: Own one Oracle run, wait for terminal status, and avoid impatience-driven reruns or model downgrades.
...
- Default to `--engine api --model gpt-5.4-pro` unless the parent or user explicitly overrides that.
...
Patience policy for `gpt-5.4-pro`:
