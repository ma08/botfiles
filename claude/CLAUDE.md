...
- Treat leading ASCII-hyphen runs on Codex thread titles as user-managed visual dividers. Preserve the exact existing generic prefix across renames. For a confidently identified unprefixed orchestrator, umbrella, parent, bucket, or group-overview thread, add the default `------ ` prefix only when no confirmed peer in that group is already prefixed; never remove, move, normalize, or deduplicate generic dividers automatically.
...
- For agent-driven Oracle reviews with prompts plus literal text or source files, prefer `~/pro/botfiles/bin/oracle-vm` whenever `research-cpu-01-ts` and its remote wrapper are healthy. This route keeps long browser automation on the GCP VM. Keep plain `oracle` host-local for predictable manual use, explicit local requests, VM outages, and Mac-only context.
- Do not transparently redirect arbitrary plain `oracle` shell invocations. `oracle-vm` must preflight the remote route, refuse ambiguous directories, globs, secrets, and binary attachments, preserve explicit model and engine choices, and return the remote slug plus reattach or harvest commands. If it fails before submission, surface the failure before deliberately choosing the local wrapper.
...
- For UI-only work that a remote Codex Desktop task must perform through the Mac task-scoped in-app Browser or native Computer Use, use `mac-ui-worker`. Bind one Mac worker to the exact source host and task IDs, reuse it for serial jobs from that source only, and route every job and receipt by stored task IDs plus a unique job ID. Each job declares exactly one UI surface and arrives as one complete exhaustive envelope. After acknowledgment, routine clarification, login, MFA, user-controlled credential entry, exact approval, and handoff stay between Sourya and the worker by default. The only approval exception is an exact single-use source delegation visibly included in the complete initial envelope. Source callbacks are only for declared dependencies, material source-owned decisions, cancellation, declared exceptional milestones, and the terminal sanitized receipt. Do not build or guess from a shared worker pool.
- Creating a Mac worker still requires explicit user authorization and native Codex Desktop project/task tools. If the transport or declared UI surface is unavailable, fail closed rather than writing app state, switching surfaces, or substituting another automation mechanism.
...
- Require the helper's composer preflight to be safe before execution, and claim forwarding only when its receipt says `outcome: delivered` with `delivered: true`. Never treat `executed`, staging, or Enter alone as delivery.
- For long or multiline Codex prompts, `send-zellij-message` adds a delayed confirm Enter only when the intended text remains visibly staged after the first Enter.
- If the helper reports unsafe or unverified state, stop automated retries and inspect manually. Never clear, append to, or submit pre-existing composer text through the helper.
- Treat visible Claude Code autocomplete as unsafe by default. Use `--replace-claude-suggestion` only after the human confirms it is a suggestion. Require the flagged preview to report `preview_probe_required`; execution may stage text only after its non-text cursor probe reports `verified_empty_buffer`. A real buffered draft must be restored unchanged and blocked.
- In non-interactive shells without `XDG_RUNTIME_DIR`, `send-zellij-message` may select `/run/user/<uid>` only when the default zellij namespace has no active exact target and the systemd namespace does. Require the receipt to record the selected namespace; never switch away from an active default target.
