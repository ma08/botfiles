...
- If the Oracle result is needed for the next step, prefer the foreground `oracle-awaiter` custom subagent and invoke it explicitly when you want to guarantee that path. Let it own one Oracle run and wait for a terminal status before handing the result back. The subagent should launch the same GPT-5.5 Pro browser/default wrapper path and apply the same repeated-failure-only fallback rule.
