...
- On the ML VM (`Azure-A100-GPU-VM`), when the user explicitly asks for ChatGPT GPT-5.5 Pro / 5.5 Pro through Oracle, use browser manual-login mode with the local Chrome wrapper and model picker: `oracle --engine browser --browser-manual-login --browser-chrome-path "$HOME/pro/botfiles/bin/oracle-chrome-linux" --model "5.5 Pro" --browser-model-strategy select -p "<prompt>" [--file ...]`. This is the verified path for `gpt-5.5-pro[browser]`; keep `gpt-5.4-pro` and normal Oracle runs on the Azure/API route unless ChatGPT browser behavior is explicitly requested.
