...
- In Claude Code, do not introduce hook-based Oracle waiting or completion enforcement for now. Keep Oracle control in normal session instructions rather than `Stop`, `agent`, or other hook workflows.
...
- In Claude Code, prefer the main conversation or an ordinary foreground subagent pattern for Oracle-related waiting, not a hook-managed flow.
