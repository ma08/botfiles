...
async function getAllContent(pageId) {
  ...
  for (const block of allBlocks) {
    ...
    if (type === 'transcription' || type === 'unsupported') {
      console.log('[' + type.toUpperCase() + ' - skipped]');
      continue;
    }
    ...
    // Handle nested blocks safely
    if (block.has_children && type !== 'child_page' && type !== 'transcription') {
      try {
        const children = await notion.blocks.children.list({ block_id: block.id });
        for (const child of children.results) {
          const ctype = child.type;
          if (ctype === 'transcription' || ctype === 'unsupported') continue;
        ...
        console.log('  [nested content not readable]');
      }
    }
  }
}
